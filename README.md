...
1. Clone the repo
1. Add your compatible file into the repo or know the path to it
...
```
{
    ...
    "link_text": ["go to section ", "turn to section "],
    "last_section": "200",
    "end_of_adventure_text": ""
}
```
...
* "link_text": is also super important, this tells the pathing tool which text phrases are used before the section number the reader needs to jump to next (NOTE: no need to added capitalised versions, the tool takes care of that).
...
* "end_of_adventure_text": this is for PB2 and ePUB files specifically but can be used regardless of file type.  The tool will look for the supplied phrase and will end the test run, this is used when it is not easy to remove parts of the file after the last section which will confuse the test tool (NOTE: this needs to have the correct capitalisation).
